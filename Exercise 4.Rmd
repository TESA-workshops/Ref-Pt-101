...
Here we look an operating model (OM) from Exercise 3 where growth is time-varying. The objective of this exercise is to explore $F_\text{MSY}$ and $B_\text{MSY}$ proxy reference points based on Spawning Potential Ratio (SPR), which represents the relative per-recruit survival under fishing compared to under no fishing. A major difference between SPR-based reference points and MSY or B0-based reference points is they do not rely on estimated of recruitment parameters. 
