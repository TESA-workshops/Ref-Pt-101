...
However, we note that the $F_\text{MSY}$ estimate is dependent on the estimate of $h$. If the $h$ is uncertain or if there are few stock-recruit points at low biomass to inform recruitment dynamics at low biomass (e.g., as in this case, see **Figure 1**, where there are no observations at low biomass) then we might consider the estimate of $F_\text{MSY}$ unreliable. In these cases, $F_\text{X%SPR}$ can serve as a proxy for $F_\text{MSY}$ that doesn't rely on assumptions of recruitment dynamics. 

However, an assumption about stock productivity is still required to select the percent of SPR to use as a proxy for $F_\text{MSY}$. In this case, it turned out that the assumption of $h = 0.8$ suggested that the stock was fairly resilient and the resulting $F_\text{MSY}$ estimate was equivalent to approximately 35% SPR. The choice of appropriate value for X and whether to use MSY or SPR-based reference points will ultimately depend on multiple factors including the validity of assumptions of the SRR and stock productivity, remembering that $M$ is also an important component of productivity and is often confounded with $h$.
