...
TODO: ADD EXAMPLE WITH TIME-VARYING M, MAYBE AS A SEPARATE EXERCISE
...
Here we look at the same operating model (OM) from Exercise 4 where growth is time-varying. The objective of this exercise is to explore the influence of the time period selected to describe growth on reference points and to demonstrate that when the Beverton-Holt stock recruitment parameters ($\alpha$ and $\beta$) are constant and a component  of the unfished SSB-per-recruit ($\phi_0$) changes (i.e., $weight$-$at$-$age$ in this case), the steepness ($h$) and therefore reference points must also change.
...
The SRR is plotted below with model-estimated Beverton-Holt $\alpha$ and $\beta$ parameters. For this exercise we assume for simplicity that selectivity is equal to the maturity ogive and is constant over all years. We assume that $weight$-$at$-$age$ varies annually and here we look at how the changes in $weight$-$at$-$age$ influence $B_0$ and $B_\text{MSY}$.
...
We explore the influence of using the mean weight-at-age from 

a) the first five years of the time series, and 
b) the last five years of the time series
...
The change in weight-at-age from the first five years of the time series to the last five years results in a decrease in $\phi_{E0}$ of about 32%.
...
The change in weight-at-age from the first five years of the time series to the last five years results in a decrease in $SSB_\text{MSY}$ of about 32%. 
...
<i><b>Figure 3. Stock-recruit pairs and model-estimated stock-recruit relationship with vertical dashed lines at the *B*~MSY~ values and vertical solid lines at the *B*~0~ values, calculated from $\phi_{E0_{first5}}$ (red) and $\phi_{E0_{last5}}$ (purple)</i></b> 
<br>

We can see the influence of temporal changes in a single component of $\phi_{E0}$ (weight-at-age) on the equilibrium reference point calculations in **Figure 3**. We may also have temporal changes in other productivity parameters (e.g., recruitment, maturity-at-age, $M$) that also influence the equilibrium reference points. 
...
In the exercise above, we have assumed that the Beverton-Holt SRR parameters $\alpha$ and $\beta$ are constant and the SRR is a fixed curve. When this assumption is made and we have changes in $\phi_{E0}$ over time, the $h$ and $R_0$ also also change over time if the SRR is to remain fixed. This can create some challenges in the interpretation of the SRR when it is parameterized in terms of $h$, $R_0$, and $\phi_0$. See Miller and Brooks (2021) for a detailed analysis of time-varying productivity and discussion of its impacts on interpretation of the SRR when it is parameterized in terms of steepness and $R_0$.
...
With a fixed SRR, parameterized in terms of $\alpha$ and $\beta$, we calculated $SSB_0$ above using a $\phi_{E0}$ calculated with weight-at-age over the first five years of the time series and the last five years of the time series. This resulted in two values of the equilibrium unfished SSB ($SSB_0$) as:
...
From Exercise 3 we saw $h = \frac{\alpha\phi_{E0}}{4+\alpha\phi_{E0}}$. We can now calculate $h$ over the first five years and over the last five years using phi_{E0}.
...
h_last5
```
Here we can see that $h$ of the SRR is 0.8 when calculated using weight-at-age over the first five years and 0.736 when using weight-at-age over the last five years. $h$ is a productivity parameter that relates to the resilience of the stock. When productivity changes (e.g., weight-at-age here) $h$ changes, even though the SRR curve is fixed.

This occurs because the change in weight-at-age influences $\phi_{E0}$, which results in different estimates of $SSB_0$ and $R_0$ on the SRR (see **Figure 4**). Since the definition of $h$ is the proportion of $R_0$ produced at 0.2 $B_0$ (Mace and Doonan 1988; see the dashed vertical reference lines at $0.2B_0$ in **Figure 4**), the change in weight-at-age influences $h$. When a SRR is parameterized in terms of $h$, $R_0$, and $\phi_0$, the time period associated with the biological and fishery parameters must be fixed to some point in time. 

In the OM used here, this time period was fixed to the average over the first five years which is why the value of $h$ for `h_first5` was 0.8 and matches the $h$ assumed in the OM conditioning. The choice of using a 5-year time period when estimating the SRR in the model was an assumption made in the Open MSE software [(openMSE)](https://openmse.com/) which uses an average over the first $A_{50}$ years of data where $A_{50}$ = approximate age at 50% maturation ~ five years for the herring stock used here. 

*I think this is too much detail about what openMSE was doing. I also need to think about this a bit more. I'll look at the Miller paper and at openMSE.* 
