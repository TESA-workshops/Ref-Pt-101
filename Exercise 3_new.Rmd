...
Here, we will use the `MSYcalc()` function that we explored in Exercise 2 to calculate $F_{MSY}$, $SSB_{MSY}$, and $SSB_0$ for various operating models (OMs) with different model assumptions for the same stock. By the end of the exercise you will get a feel for:
...
Seven OMs were conditioned to age-composition data for the catch and a survey index for a herring stock (1968-2020) using various assumptions for $h$ and $M$ and future $selectivity$. These operating models were fit specifically for demonstration purposes for this exercise. A dataframe **D** is provided that contains the relevant data for the exercise. Here we assume all seven OMs represent the same stock but with different assumptions on $h$, $M$, and $selectivity$.

The models were conditioned using a Beverton-Holt stock-recruit relationship (SRR). A base model has an assumed steepness ($h$) of 0.8, a constant natural mortality rate ($M$) of 0.3 for all years and ages and $selectivity$ equal to the maturity ogive. Additional models were fit as sensitivities with alternative $h$ (0.6 or 0.95), and alternative $M$ (0.2 or 0.4) and alternative selectivities. In this exercise we look at the influence of the assumptions of these model parameters on reference point estimates.

The steepness ($h$) of the stock recruitment relationship is the proportion of unexploited recruitment, $R_0$, produced at 0.2 $B_0$ (Mace and Doonan 1988). The ($h$) parameter is bounded between 0.2 (a straight line, recruitment is proportional to SSB) and 1 (a horizontal line, recruitment is constant across SSB) as shown in **Figure 1** below. See slides XX of the PPT for more information.
...
```
<i><b>Figure 1. Predicted relative recruitment (R/R~0~) vs. relative spawning stock biomass (B/B~0~) for various values of steepness (h) of the stock-recruit relationship, where h is the relative recruitment when the relative biomass is 0.2 (vertical dashed line)</i></b> 
...
*Note*: The SRRs in these operating models are parameterized using a specified fixed $h$ and the mean $\phi_0$ over the first 5 historical years, and $R_0$ is estimated during the OM conditioning. This was done for <u>demonstration purposes only</u> to explore the influence of $h$ on reference point estimates. The practice of fixing $h$ is generally not recommended for stock assessment. Punt (2008) suggests to never base $B_{MSY}$ on an estimate from a stock assessment when $h$ is fixed (and not estimated) in the assessment. Common practice is to use priors for $h$ when fitting assessment models (e.g., Myers et al. 1999; Mangel et al. 2010). 
...
## Calculate $F_{MSY}$, $B_{MSY}$, and $B_0$ for each OM. 
...
Using the `MSYcalc()` function, we provide the at-age vectors for *M* (`M`), weight-at-age (`waa`), maturity (`mat`), selectivity (`sel`) as well as the Beverton-Holt \alpha (`BHa`) and \beta (`BHb`) parameters. The code for the `MSYcalc()` function is in the file "source_ftns.r". The function calculates:

* $F_{MSY}$ as the equilibrium fishing mortality rate ($F_e$) that maximizes yield over a vector of $F_e$ values, 

* $MSY$ as the equilibrium yield from fishing long-term at $F_{MSY}$,

* $B_{MSY}$ (i.e., $SSB_{MSY}$) as the equilibrium SSB from fishing long-term at $F_{MSY}$,
...
If a stock is more resilient to fishing pressure (i.e., higher $h$) then we would expect that the stock can be fished harder without risk to recruitment than a stock with lower resilience. We can see from **Table 1** and **Figure 4** that $F_{MSY}$ increases with increasing $h$.
...
We can see from **Figure 3** that the stock can be brought to a lower biomass without a significant decline in predicted recruitment when $h$ is higher. With higher $h$, we saw that $F_{MSY}$ is higher, and with a higher *F* we would expect to have a lower equilibrium *B* from fishing at a higher *F*. We can see this relationship in **Table 1** and in a plot of Yield vs. equilibrium SSB (**Figure 5**) where $B_{MSY}$ is lower for higher $h$.
...
<br>
We saw that as $h$ increases, $B_{MSY}$ decreases (and $B_0$) increases. The level of depletion of $B_{MSY}$ (i.e., the ratio $\frac{B_{MSY}}{B_0}$ therefore decreases with increasing *h* (**Table 2**).

The reliability of estimates of $B_{MSY}$ depends on how well $h$ is estimated. Estimates of $h$ can be obtained from meta-analyses (e.g., Thorson et al. 2020) or from life-history characteristics (e.g., Wiff et al. 2018) and these estimates can be can be used to guidance for prior distributions for $h$. Given the sensitivity of the $B_{MSY}$ calculation to $h$, alternative reference points (e.g., proxies for $B_{MSY}$) may be more reliable when $h$ is highly uncertain.
...
In general, if a stock has a higher $M$ then it is more productive and can be subjected to a higher *F*. We see this in **Table 1** and a plot of yield relative to *F* (**Figure 8**) where $F_{MSY}$ increases with increasing $M$. 
...
The plot of yield vs. equilibrium SSB (**Figure 9**) shows a slightly higher estimate of $B_{MSY}$ with higher $M$ and the maximum yield (i.e., $MSY$ itself) is influenced most strongly by $M$. There is no clear relationship between $B_{MSY}$ and $M$ as demonstrated in **Table 2**.
...
Lines through the origin with slope $\frac{1}{\phi_0}$ are shown with the three SRRs in **Figure 10** and intersect the SRRs at a similar SSB. The $B_{0}$ values increase with increasing *M*. Although the ratios of $\frac{B_{MSY}}{B_0}$ decrease slightly with increases in $M$, the changes are not as extreme as those seen for $h$ (**Table 2**).
...
Estimates of $M$ influence estimates of $B_0$ and the MSY reference points. In general a higher $M$ results in a higher estimate of $B_0$, a higher $F_{MSY}$, and a lower ratio of $\frac{B_{MSY}}{B_0}$ (He et al. 2011; Szuwalski and Punt 2012; Punt et al. 2021). Although the magnitude of the MSY reference points are influenced by $M$, estimates of $B_{MSY}$ depend more strongly on $h$ and are not impacted substantially by $M$ when $h$ is correctly specified (Punt et al. 2021). 
...
The influence of selectivity on the reference points estimates is determined by the relationship between selectivity and the mature ogive. A stock will be more resilient to fishing pressure if adults are given the opportunity to spawn before they are vulnerable to the fishery. As such, we would expect that as the selectivity curve is shifted to the right of the maturity ogive, it would increase $F_{MSY}$ and decrease $B_{MSY}$. And as the selectivity curve is shifted to the left of the maturity ogive, it would decrease $F_{MSY}$ and increase $B_{MSY}$. This is demonstrated in **Table 2**, **Figure 11**, and **Figure 12** below. The changes in selectivity also have some influence on $B_0$ and the ratio of $\frac{B_{MSY}}{B_0}$, such that both metrics decrease with shifts in selectivity to the right of the maturity ogive and increase with shifts to the left of the maturity ogive (**Table 2**).
