...
where $B$ is biomass, $r$ is the intrinsic rate of population growth, $K$ is the carrying capacity, $C$ is the catch, and $t$ is a discrete time period (one year in this case). $B_{MSY}$ is then calculated as $K/2$ and $F_{MSY}$ is calculated as $r/2$, where $F$ is the instantaneous fishing mortality rate (Hilborn and Walters 1992, Ch. 8; see slides *XXX* of the PPT).
...
P <- data.frame("Year"=DF$Year, 
    ...
    "F"=-log(1-(DF$Catch/DF$Model_biomass))) 
...
<i><b>Figure 4. Time series plot of biomass for the historical time period and 100 projected years for four different constant fishing mortality rates</i></b> 

Fishing constantly at any fishing mortality rate results in the stock coming to some equilibrium biomass $B$, ranging between $B = 0$ and $B = K$. The Schaefer model predicts that the stock will eventually come to carrying capacity $K$ under no fishing, and to $B_{MSY}$ under a constant fishing mortality rate of $F_{MSY}$, as shown in the plot.

We can see this by comparing final year equilibrium values from the above numerical exercise with the analytical predictions of the Schaefer model (i.e., $B_{MSY} = K/2$ and $F_{MSY} = r/2$). The numerical and analytical solutions for $K$ and $B_{MSY}$ are identical.
...
<i><b>Figure 5. Plot of yield vs. Fishing Mortality rate ($F$) with a vertical line at $F_{MSY}$ where yield is maximized</i></b> 

**Figure 5** shows the final year catch (i.e., yield) under each of the tested fishing mortality rates. It confirms that long-term yield is maximized by fishing at $F_{MSY}$, indicated by the blue dashed line.

Note: The figure above (an inverted parabola) suggests that we could use calculus to find the fishing mortality rate that maximizes yield (i.e., the derivative of yield with respect to $F$ = 0; See slides **XXX** of the powerpoint). This is indeed how most numerical searches for $F_{MSY}$ work. For the purposes of these exercises, however, we used a simpler grid search. This most clearly illustrates the concept of maximizing yield, but please be aware that this is the least efficient means of finding $F_{MSY}$!
