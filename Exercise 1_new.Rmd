...
where $B$ is biomass, $r$ is the intrinsic rate of population growth, $K$ is the carrying capacity, $C$ is the catch, and $t$ is a discrete time period (one year in this case). $B_{MSY}$ is then calculated as $K/2$ and $u_{MSY}$ is calculated as $r/2$, where $u$ is annual harvest rate (Hilborn and Walters 1992, Ch. 8; see slides *XXX* of the PPT).
...
<i><b>Figure 3. Time series plot of biomass and catch for the historical time period and projected over 100 years using the harvest rate from the terminal year </i></b> 

**Figure 3** shows the historical population biomass to the left of the dashed vertical line, and the projected biomass to the right. The population reached equilibrium under the constant harvest rate quite quickly. Just to extend this a bit, let's do the same with four different harvest rates: $u = 0$, $u =$ `terminalU`, $u = U_{MSY}$, and a high harvest rate that would crash the population ($u = r$). Note that $U_{MSY} = r/2$ from the fitted model.
...
<i><b>Figure 4. Time series plot of biomass for the historical time period and 100 projected years for four different constant harvest rates</i></b> 

Fishing constantly at any harvest rate results in the stock coming to some equilibrium biomass $B$, ranging between $B = 0$ and $B = K$. The Schaefer model predicts that the stock will eventually come to carrying capacity $K$ under no fishing, and to $B_{MSY}$ under a constant harvest rate of $U_{MSY}$, as shown in the plot.

We can see this by comparing final year equilibrium values from the above numerical exercise with the analytical predictions of the Schaefer model (i.e., $B_{MSY} = K/2$ and $u_{MSY} = r/2$). The numerical and analytical solutions for $K$ and $B_{MSY}$ are identical.
...
We've seen that $B_{MSY}$ is an analytical property of the Schaefer model $\left(K/2\right)$, which can be reproduced numerically by running out the model under a constant harvest rate of $u_{MSY}$. But where did $u_{MSY}$ come from? Next, we'll repeat Step 2 over a range of harvest rates from 0 to 1 in increments of 0.001 and show that the constant harvest rate $u$ that maximizes long-term yield is the same as the estimate of $u_{MSY}$ from Section 1.

The two-parameter Schaefer model produces a symmetrical yield curve. More complex, non-symmetrical versions of this model exist (e.g., the Pella-Tomlinson model; Hilborn and Walters 1992) and, indeed, similar yield curves are produced by age-structured models. The important concept from this exercise is that $u_{MSY}$ (or its instantaneous equivalent $F_{MSY}$, which we'll see in Exercise 2) is the long-term harvest rate that would maximize yield in the long-term under stable conditions.

In the exercise below, we identify $u_{MSY}$ numerically by running out the model under a sequence of harvest rates and identifying the value of $u$ that maximizes yield in the long term.
...
<i><b>Figure 5. Plot of yield vs. harvest rate (u) with a vertical line at $u_{MSY}$ where yield is maximized</i></b> 

**Figure 5** shows the final year catch (i.e., yield) under each of the tested harvest rates. It confirms that long-term yield is maximized by fishing at $u_{MSY}$, indicated by the blue dashed line.

Note: The figure above (an inverted parabola) suggests that we could use calculus to find the harvest rate that maximizes yield (i.e., the derivative of yield with respect to $u$ = 0; See slides **XXX** of the powerpoint). This is indeed how most numerical searches for $u_{MSY}$ or $F_{MSY}$ work. For the purposes of these exercises, however, we used a simpler grid search. This most clearly illustrates the concept of maximizing yield, but please be aware that this is the least efficient means of finding $u_{MSY}$!
