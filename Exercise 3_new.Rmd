...
In general, if a stock has a higher $M$ then it is more productive and is reslient to a higher *F*. We see this in **Table 1** and a plot of yield relative to *F* (**Figure 8**) where $F_\text{MSY}$ increases with increasing $M$. 
...
Estimates of $M$ influence estimates of $B_0$ and the MSY reference points. In general a higher $M$ results in a **lower** estimate of $B_0$, a higher $F_\text{MSY}$, and a lower ratio of $\frac{B_\text{MSY}}{B_0}$ (He et al. 2011; Szuwalski and Punt 2012; Punt et al. 2021). *Although the magnitude of the MSY reference points are influenced by $M$, estimates of $B_\text{MSY}$ depend more strongly on $h$ and are not impacted substantially by $M$ when $h$ is correctly specified (Punt et al. 2021). I don't agree with this. BMSY is strongly influenced by M, and it is not possible to know whether h is correctly specified. Steepness and M are often confounded. I would delete this bit. There may be some subtle estimation effects that Punt is talking about, but the analytical correlation between Bmsy and M is negative*. 
