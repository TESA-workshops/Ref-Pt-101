...
* Calculate equilibrium unfished biomass ($B_\text{0}$) and equilibrium MSY reference points ($B_\text{MSY}$, $F_\text{MSY}$, $MSY$) when a stock-recruitment relationship is assumed
...
<!-- NOTE TO TIM: I overwrote BI$sel with values from a logistic curve. See get-sel-pars.R. You shouldn't need to run it as I overwrote the Ex2 rda file but have saved the original. Please could you add the estimate of steepness and R0 from the OM to the dataset? -->
...
At any constant equilibrium fishing mortality $F_{e}$, the equilibrium survivorship-at-age $l_a$ is a simple modification of Equation 1, accounting for fishing mortality and vulnerability-at-age $v_{a}$:
...
*Note:* The `survivorship_F()` function in source_ftns.R can be used here to calculate $l_a$ for any $F_e$ as shown in Equation 5. To obtain unfished $l_{0_{a}}$, set f = 0 in the `survivorship_F()` function.
...
(Walters and Martell 2004, Box 3.1). We assume that the model has estimated the leading parameters $R_{0}$ and steepness $h$. The $\alpha$ and $\beta$ parameters are then derived as:
...
# NEED THE STEEPNESS FROM THE OM. It would also be good to have R0 to show consistency
#   between the BHa and BHb parameters and R0 and Steepness. Just to show you only need
#   two estimated leading parameters to get everything else
...
<i><b>Figure 1. Model estimated recruitment (age-0 recruits in millions) vs. spawning stock biomass (in kt) with the stock-recruit relationship (SRR; black line), and a line with slope $\frac{1}{\phi_{E0}}$ (dashed line) that intersects the SRR at $SSB_0$ (vertical dashed line)</i></b> 

## Estimate the MSY reference points ($F_\text{MSY}$, $SSB_\text{MSY}$, $MSY$)

To estimate $SSB_\text{MSY}$ (i.e., $B_\text{MSY}$), we need an estimate of equilibrium recruitment at $F_\text{MSY}$. To identify $F_\text{MSY}$, we therefore need to calculate equilibrium recruitment $R_e$ over a range of equilibrium fishing mortality rates $F_e$. 
...
then long-term equilibrium yield $Y_e$ is simply the product of the number of recruits and the yield-per-recruit:
...
5) Calculate $SSB_{MSY}$ as the product of $R_e$ and $\phi_{F}$ evaluated at $F_{MSY}$ (Equation 12);
...
<i><b>Figure 2. Equilibrium yield vs. equilibrium SSB over a range of $F$ values. MSY is the maximum yield (horizontal dashed line) and occurs at SSB = $SSB_\text{MSY}$ (vertical dashed line)</i></b> 
...
<i><b>Figure 3. Equilibrium yield vs. equilibrium $F$ over a range of $F$ values. MSY is the maximum yield (horizontal dashed line) and occurs at F = $F_\text{MSY}$ (vertical dashed line)</i></b> 
