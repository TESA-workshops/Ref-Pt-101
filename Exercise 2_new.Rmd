...
## Exercise 2: Estimating unfished biomass and MSY reerence points using an age-structured model with a Beverton-Holt stock-recruit relationship
...
* Calculate equilibrium unfished biomass (*B*~0~) and equilibrium MSY reerence points (*B*~MSY~, $F$~MSY~, *MSY*) when a stock-recruitment relationship is assumed
...
# Beverton-Holt parameters a and b (model estimated and used for plotting and reerence point calculations)
BHa <- data$BHa
BHb <- data$BHb
...
In order to calculate MSY-based reerence points, we also need to calculate long-term equilibrium population behaviour under constant fishing mortality rates.
...
To get $R_\text{MSY}$, we can use the Beverton-Holt stock-recruit function (see footnote for Ricker equations). The Beverton-Holt stock-recruit function is defined by two parameters $\alpha$ and $\beta$ (respectively called BHa and BHb in our data above). 
...
For more on derivation of \alpha, see Forrest et al. 2010 and reerences therein. Once we have an estimate of \alpha, we can obtain \beta by rearranging Equation 8:
...
*Tim: I lined up these steps with the equations above, which are a slight rejigging of what you had. I think a little easier to follow. I tried to make the functions match the steps but please check*

*Please note we are not estimating anything here. We already have estimates of alpha and beta so we are just deriving or calculating the rest*
...
The MSY reerence points are plotted below the the yield curves:

```{r,figures-side, warning = F, fig.show="hold", out.width="75%"}

# Plot Yield vs. Equilibrium SSB with reerence lines at SSBmsy and MSY
...
# Plot Yield vs. F with reerence lines at Fmsy and MSY
...
The above calculations apply to iteroparous species, where we need to account for  mortality of mature fish as they age and spawn through the years. We do not have to account for this for semelparous species, such as Salmonids, which spawn only once in their lietime. For these species, the population can be modeled using only the stock-recruit relationship,  often the Ricker (Ricker 1954) model. MSY-based reerence points can be estimated directly from the alpha and beta parameters of the Ricker or Beverton-Holt stock-recruit models (see Hilborn and Walters 1992, Table 7.2). We do not have time to explore estimation of Salmonid reerence points in these webinars but see **XXX** for more information. *RF has asked a colleague for some key refs*
