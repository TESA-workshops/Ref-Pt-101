...
Seven operating models were conditioned to age-composition data for the catch and a survey index for a herring stock (1968-2020) using various assumptions for *h* and *M* and future selectivity. An R object is provided for each operating model that contains the relevant data for the exercise. The models were conditioned using a Beverton-Holt stock-recruit relationship (SHH) with assumed *h* (0.6, 0.8, or 0.95) over the first 5 years, and a constant *M* (0.2, 0.3, of 0.4) for all years and ages. In this exercise we look at the influence of the assumptions of *h*, *M*, and selectivity on estimates of *SSB*~MSY~ and *SSB*~0~.
...
T0<-data.frame(OM,h,M,Selectivity=sel,Fmsy=NA,SSBmsy=NA,SSB0=NA)
T0[,1:4] %>% kbl(align=c(rep('c',7))) %>% kable_styling()
```

```{r, echo=F,warning=F}
mat <- c(0, 0.001075732, 0.040440821, 0.628325999, 0.982591351, 0.999470529, 0.999983827, 1, 1, 1, 1, 1)             

SDF <- data.frame(Age=rep(0:11,3),Selectivity=c(mat,c(mat[2:12],1),c(0,mat[1:11])), Scenario = rep(c('mat','low','high'),each=12))
...
1. Estimate *F*~MSY~, *SSB*~MSY~, and *SSB*~0~ for each of the 7 OMs. The code to extract the data from the OMs is below. The reference point calculations can be added to the loop below.
