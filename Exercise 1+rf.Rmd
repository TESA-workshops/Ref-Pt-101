...
To do this, we first need to fit a population model to the swordfish data to estimate the biomass, harvest rate and model parameters. We'll start with a simple Schaefer, or Surplus Production model (REF). The Schaefer model is: *B*~*t*+1~ = *B*~*t*~ + *rB*~*t*~ (1-*B*~*t*~/*K*) - *C*~*t*~
...
for(y in (nyr+1):fpyr)
  { # loop over projection years
    ...
    P[y,2] <- lastB + r*lastB*(1-lastB/K)-lastC # Projected Biomass from the Schaefer model
    P[y,3] <- lastB*terminalU # Projected Catch from the Schaefer model
}
...
for(i in 1:length(u)){ #loop over u
  for(y in 1:pyr){     #loop over years
    ...
    P2[y,i,1] <- lastB + r*lastB*(1-lastB/K)-lastC # Estimated Biomass from the Schaefer model
    P2[y,i,2] <- lastB*u[i] # Estimated Catch from the Schaefer model
  }
}
...
We've seen that *B*~MSY~ is an analytical result of the Schaefer model (*K*/2) that can be reproduced numerically by running out the model under a constant harvest rate of *U*~MSY~. Next, we'll repeat Step 2 over a range of harvest rates from 0 to 1 in increments of 0.00 and show that the *U* that maximizes yield is the same as the estimate of *U*~MSY~ (= *r*/2) from Section 1.


```{r,warning=F}
u <- seq(0,1,0.01)
P3 <- array(0,dim=c(100,length(u),2),dimnames = list(1:100,u,c("Biomass","Catch")))
...
for(i in 1:length(u)){
  for(y in 1:100){
    ...
    P3[y,i,1] <- lastB + r*lastB*(1-lastB/K)-lastC #Biomass
    if(P3[y,i,1]<0.000001) P3[y,i,1] <- 0.000
    # Get the catch for next year
    P3[y,i,2] <- lastB * u[i]  # Catch = qEB, but qE = F (assume q=1)
  }
}
...
TODO: Add some text on problems with MSY and shift to thinking about it as a limit
