...
If we were doing a real stock assesment, we would refit the model two ways: once assuming asymptotic selectivity and again allowing for dome-shaped selectivity. We might even allow for time-varying selectivity. However, for the purpose of isolating the effects of selectivity on reference points, we will just use the two selectivities above and hold all the other parameters constant. But be aware that refitting the model with a different selectivity curve would result in changes in several other parameter estimates.
...
Age structured models track the numbers of fish over age and time. Therefore, to calculate spawning biomass, we need to track the weight at age and maturity at age, as well as mortality at age. 
