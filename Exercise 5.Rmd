...
h_last5
```
Here we can see that $h$ of the SRR is 0.8 when calculated using weight-at-age over the first five years and 0.736 when using weight-at-age over the last five years. $h$ is a productivity parameter that relates to the resilience of the stock. When productivity changes (e.g., weight-at-age here) $h$ changes, *and therefore so does the SRR. If steepness changes then alpha and beta must change, so the SRR must change surely.*
...
**In the OM used here, this time period was fixed to the average over the first five years which is why the value of $h$ for `h_first5` was 0.8 and matches the $h$ assumed in the OM conditioning. The choice of using a 5-year time period when estimating the SRR in the model was an assumption made in the Open MSE software [(openMSE)](https://openmse.com/) which uses an average over the first $A_{50}$ years of data where $A_{50}$ = approximate age at 50% maturation ~ five years for the herring stock used here.** *I think this is too much detail about what openMSE was doing. I also need to think about this a bit more. I'll look at the Miller paper and at openMSE.* 
