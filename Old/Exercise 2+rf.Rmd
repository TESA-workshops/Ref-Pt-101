...
## Exercise 2: Proxy reference points using an age-structured model with no stock-recruit relationship -- maybe move to a later exercise
...
Add preamble with equations describing the purpose of the exercises

Think about whether this is the best second exercise. I think we should deal with FMSY from a standard S-R approach first, then talk about proxies. As noted, I'm a bit wary of using an example where we assume R0 at BMSY. I guess it can happen when R0 is defined as the median recruitment and the S-R relationship is pretty flat, but I think it's confusing from a 101 point of view.


We will use an operating model object from [openMSE](https://openmse.com/) that has been conditioned on age-composition data for the catch and three survey indices for a haddock stock (1971-2018). Natural mortality, *M*, was assumed to be 0.2 *y*^-1 and no stock-recruit relationship was assumed. The stock-recruit pairs are plotted below and saved in the dataframe **SR**. The biological data and selectivity for age classes 0 to 9+ are saved in the dataframe **BI**.
...
Note: The `survivorship_F`() function in source_ftns.R can be used here.

```{r,warning=F,message=F}
# Note to be deleted: I added the default f=0 into the function call, just to be clear. I also changed l0 to surv0 because it looked like the number 10
surv0 <- survivorship_F(f=0,M=BI$M,n_ages=nrow(BI))
phi0 <- sum(surv0*BI$waa*BI$mat)
phi0
```

2. Assume the the equilibrium unfished recruitment (*R*~0~) is the median recruitment in the time series and estimate the unfished equilibrium SSB (*SSB*~0~). 

The stock-recruit plot is below. Add a horizontal line at the median recruitment and straight line with slope 1/$\phi$~0~ to show the predicted *SSB*~0~ occurs at the intersection of the median recruitment line and line with slope 1/$\phi$~0~.

Note to be deleted: It bugs me a bit that the predicted SSB0 should be so much lower than the maximum SSB observations and it also puts us in a very weird situation where R0 = RMSY according to the next section below. I don't know if median is the right metric to use. Is this what was used before? This looks more to me like some thought needs to be put into fitting two different S-R curves, as I suspect all those big recruitments are recent. 
...
ggplot(SR)+geom_point(mapping=aes(y=Rec,x=SSB)) + theme_classic() +
  ...
  geom_abline(intercept=0, slope=(1/phi0),colour="red",linetype=2)+
